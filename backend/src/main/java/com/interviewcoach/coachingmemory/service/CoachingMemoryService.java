...
public class CoachingMemoryService {
    ...
    private static final String SYSTEM_PROMPT_TEMPLATE = """
            ...
            JSON 结构必须严格如下：
            {
              "observedStrengths": ["强项描述"],
              "observedWeaknesses": ["短板描述"],
              "recurringProblems": ["重复问题"],
              "verifiedExperience": ["已验证的经历"],
              "unverifiedClaims": ["未验证的声明"],
              "recommendedNextFocus": ["下一步重点"],
              "avoidRepeating": ["避免重复的内容"]
            }
    ...
    private CoachingMemoryDto generateAndSave(User user, InterviewTarget target,
        ...
        memory.setObservedStrengths(CollectionUtils.copyList(aiResult.observedStrengths()));
        memory.setObservedWeaknesses(CollectionUtils.copyList(aiResult.observedWeaknesses()));
        memory.setRecurringProblems(CollectionUtils.copyList(aiResult.recurringProblems()));
        memory.setVerifiedExperience(CollectionUtils.copyList(aiResult.verifiedExperience()));
        memory.setUnverifiedClaims(CollectionUtils.copyList(aiResult.unverifiedClaims()));
        memory.setRecommendedNextFocus(CollectionUtils.copyList(aiResult.recommendedNextFocus()));
        memory.setAvoidRepeating(CollectionUtils.copyList(aiResult.avoidRepeating()));
    ...
    private CoachingMemoryDto toDto(CoachingMemory memory) {
        return new CoachingMemoryDto(
                ...
                CollectionUtils.copyList(memory.getObservedStrengths()),
                CollectionUtils.copyList(memory.getObservedWeaknesses()),
                CollectionUtils.copyList(memory.getRecurringProblems()),
                CollectionUtils.copyList(memory.getVerifiedExperience()),
                CollectionUtils.copyList(memory.getUnverifiedClaims()),
                CollectionUtils.copyList(memory.getRecommendedNextFocus()),
                CollectionUtils.copyList(memory.getAvoidRepeating()),
